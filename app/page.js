...
// Bottom navigation bar — shown only on the home screen
function BottomNav() {
  const items = [
    {
      href: '/',
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 bg-white border-t border-aq-border"
        ...
        {items.map((item) => (
          <Link
            ...
            className={`flex flex-col items-center justify-center gap-[3px] flex-1 min-h-[56px] transition-colors duration-150 ${
              item.active ? 'text-aq-green' : 'text-aq-muted hover:text-aq-ink'
            }`}
          >
            {item.icon}
