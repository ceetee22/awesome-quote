...
export async function getSettings() {
  ...
  const [{ data: biz }, { data: zones }] = await Promise.all([
...
export async function createBusiness({ owner_id, name, contact_person_name, contact_email }) {
  ...
  if (error) console.error('createBusiness failed:', error.message)
  return data
}
...
export async function ensureBusinessFromMetadata() {
  ...
  return createBusiness({ owner_id: user.id, name, contact_person_name, contact_email: user.email || '' })
}
...
export async function saveSettings(settings) {
  ...
  // RLS (owner_id = auth.uid()) scopes the update to only the current user's business
  const { error } = await supabase.from('businesses').update(bizFields).not('id', 'is', null)
  if (error) console.error('saveSettings failed:', error.message)
  if (callout_zones) {
    // RLS scopes delete/insert to the current user's callout_zones automatically
