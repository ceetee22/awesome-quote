...
export async function getSettings() {
  ...
  return {
    ...
    day_start_minute: row.day_start_minute != null ? Number(row.day_start_minute) : 480,
    day_end_target_minute: row.day_end_target_minute != null ? Number(row.day_end_target_minute) : null,
    default_buffer_minutes: row.default_buffer_minutes != null ? Number(row.default_buffer_minutes) : 10,
    home_base_lat: row.home_base_lat != null ? Number(row.home_base_lat) : null,
    home_base_lng: row.home_base_lng != null ? Number(row.home_base_lng) : null,
  }
}
...
export async function getPlannerBacklog() {
  ...
  const { data } = await supabase
    .from('jobs')
    .select('id, customer_name, customer_address, status, accepted_at, estimated_duration, schedule_state, created_at, callout_fee, hourly_rate, labour_hours, job_items(id, type, description, joinery_type_label, fault_label, job_item_parts(sell_price, qty))')
    .eq('status', 'accepted')
...
export async function getPlannerWeekJobs(weekStart, weekEnd) {
  ...
  const { data } = await supabase
    .from('jobs')
    .select('id, customer_name, customer_address, status, scheduled_date, slot, estimated_duration, sequence_index, start_minute, schedule_state')
...
export async function dbAssignJob(jobId, { scheduled_date, slot, estimated_duration, sequence_index, start_minute }) {
  if (!supabase) return null
