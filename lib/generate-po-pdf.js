...
const COL_UNIT_R  = COL_TOTAL_R - 27      // 161
const COL_QTY_R   = COL_UNIT_R - 12       // 149
const COL_SKU_R   = COL_QTY_R - 32        // 117
const ROW_H       = 7
const CELL_PAD    = 2.5
...
export async function generatePoPdf({ job, settings, orderLines }) {
  ...
  const deliverTo = job.customer_address || settings?.home_base_address || ''
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  muted(doc)
  if (deliverTo) {
    doc.text(`Please deliver to: ${deliverTo}`, MARGIN, footerY)
  }
  doc.setTextColor(...C_SUBTLE)
  doc.setFontSize(8)
  doc.text(bizName,  PAGE_W / 2, footerY + (deliverTo ? 5 : 0),     { align: 'center' })
  doc.text(poNumber, PAGE_W / 2, footerY + (deliverTo ? 10 : 5),    { align: 'center' })
