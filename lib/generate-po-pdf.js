...
export async function generatePoPdf({ job, settings, orderLines, collectionMethod = 'pickup', deliveryAddress = '', logoUrl = null }) {
  ...
  if (logo) {
    ...
    poLogoH = Math.min(MAX_H, MAX_W / logo.aspect)
    const logoW = poLogoH * logo.aspect
    doc.addImage(logo.dataUrl, 'JPEG', MARGIN, y, logoW, poLogoH)
  } else {
    doc.setFillColor(...C_GREEN)
    doc.roundedRect(MARGIN, y, 16, 16, 3, 3, 'F')
    doc.setTextColor(...C_WHITE)
    const initials = getInitials(displayName)
    doc.setFontSize(initials.length > 1 ? 9 : 11)
  ...
  if (logo) {
    // Name suppressed — the uploaded logo already carries the wordmark
    // Contact sits below the logo with a small gap
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    muted(doc)
    if (contact) doc.text(contact, MARGIN, y + poLogoH + 7)
  } else {
