...
export async function generateQuotePdf({
  ...
  // Left: logo or AQ monogram
  let nameX = MARGIN + 20
  if (logo) {
    const MAX_W = 32, MAX_H = 14
    const h = Math.min(MAX_H, MAX_W / logo.aspect)
    const w = h * logo.aspect
    doc.addImage(logo.dataUrl, 'JPEG', MARGIN, y + (16 - h) / 2, w, h)
    nameX = MARGIN + w + 4
  } else {
  ...
  if (settings?.business_tagline) {
    ...
    muted(doc)
    doc.text(settings.business_tagline, nameX, infoY)
  }
  ...
  if (gstNumber) {
    ...
    doc.setFont('helvetica', 'normal')
    muted(doc)
    doc.text(`GST number: ${gstNumber}`, nameX, infoY)
  }
