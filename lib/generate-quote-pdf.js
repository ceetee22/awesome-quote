...
export async function generateQuotePdf({
  ...
  const MAX_LOGO_W = 54  // mm — 20% larger for better legibility
  const MAX_LOGO_H = 24  // mm — cap so wide logos don't dominate
  ...
  if (logo) {
    ...
    doc.setFontSize(initials.length > 1 ? 11 : 13)
    doc.setFont('helvetica', 'bold')
    doc.text(initials, MARGIN + logoW / 2, y + logoH * 0.62, { align: 'center' })
  }

  // Title — top-right, baseline at logo vertical centre + half cap-height (~3mm at 22pt)
  const titleY = y + logoH / 2 + 3
  if (isRevision) {
    doc.setFontSize(15)
    doc.setFont('helvetica', 'bold')
    ink(doc)
  ...
  if (!logo) {
    // Only show name/tagline when there is no logo — the uploaded logo already carries the wordmark
    doc.setFontSize(13)
    doc.setFont('helvetica', 'bold')
    ink(doc)
  ...
  // ── ITEMS TABLE (bundled — no unit prices or part detail shown to customer) ──

  // Table header row — 2 columns: Description | Total
  ...
  // Helper: draw a bundled line row
  function bundledRow(label, lineTotal) {
    doc.setDrawColor(...C_BORDER)
    ...
    doc.setFont('helvetica', 'normal')
    ink(doc)
    const maxW = COL_TOTAL_R - (MARGIN + CELL_PAD) - 25
    doc.text(label || '', MARGIN + CELL_PAD, y + 4.8, { maxWidth: maxW })
    if (lineTotal !== '') doc.text(formatCurrency(lineTotal), COL_TOTAL_R, y + 4.8, { align: 'right' })
    y += ROW_H
  }

  // One row per job item — bundled parts total only, grouped by room if rooms exist
  const activeItems = (job.items || []).filter(
    (item) => (item.parts || []).length > 0 || item.type === 'custom'
  )

  const hasRooms = activeItems.some((i) => i.room_id)
  ...
  function renderItem(item) {
    ...
    const lineTotal = parts.reduce((s, p) => s + p.sell_price * p.qty, 0)
    bundledRow(itemLabel(item), lineTotal)
  }

  if (hasRooms) {
    // Build room order from job.rooms (if present), then iterate
    const roomOrder = (job.rooms || []).map((r) => r.id)
    const byRoom = new Map()
    const noRoom = []
    for (const item of activeItems) {
      if (!item.room_id) { noRoom.push(item); continue }
      if (!byRoom.has(item.room_id)) byRoom.set(item.room_id, { name: item.room_name || 'Room', items: [] })
      byRoom.get(item.room_id).items.push(item)
    }
    // Items without a room first
    noRoom.forEach(renderItem)
    // Then room groups in creation order
    const orderedRooms = []
    for (const id of roomOrder) {
      if (byRoom.has(id)) orderedRooms.push(byRoom.get(id))
    }
    for (const [id, room] of byRoom) {
      if (!roomOrder.includes(id)) orderedRooms.push(room)
    }
    for (const room of orderedRooms) {
      // Room header row (muted, no total)
      doc.setFillColor(245, 247, 246)
      doc.rect(MARGIN, y, CONTENT_W, ROW_H, 'F')
      doc.setFontSize(8)
      doc.setFont('helvetica', 'bold')
      muted(doc)
      doc.text(room.name.toUpperCase(), MARGIN + CELL_PAD, y + 4.8)
      y += ROW_H
      room.items.forEach(renderItem)
    }
  } else {
    activeItems.forEach(renderItem)
  }

  // Combined labour and callout row
  const labourAndCallout = (labourHours || 0) * (hourlyRate || 0) + (calloutFee || 0)
  if (labourAndCallout > 0) {
    bundledRow('Labour and callout', labourAndCallout)
  }
  ...
  const ref = `Ref: JOT-${(job.id || '').substring(0, 8).toUpperCase()}`
