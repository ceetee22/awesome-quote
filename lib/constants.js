...
// Repair template fault matrix — label strings are the unique key stored in repair_templates.fault
// (FAULT_OPTIONS.value has duplicates within some types; labels are unique per type)
export const REPAIR_TEMPLATE_FAULTS = [
  {
    type: 'sliding_door',
    label: 'Sliding door',
    faults: [
      'Stiff or hard to slide',
      "Won't lock or latch",
      'Off track or jumping',
      'Broken handle',
      'Drafty or leaking',
      'Other',
    ],
...
export const TOTAL_TEMPLATE_COUNT = REPAIR_TEMPLATE_FAULTS.reduce(
  (sum, g) => sum + g.faults.length,
  0
)
