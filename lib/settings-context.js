...
import { getSettings, saveSettings, ensureBusinessFromMetadata } from './db'
...
const DEFAULT_STATE = {
  ...
  pricing_wizard_dismissed: false,
}
...
export function SettingsProvider({ children }) {
  ...
  async function load() {
    ...
    try {
      ...
      if (!data && supabase) {
        // Only call ensureBusinessFromMetadata when a session is confirmed —
        // avoids "Auth session missing" noise during unauthenticated page loads.
        const { data: { user } } = await supabase.auth.getUser()
        if (user) {
          await ensureBusinessFromMetadata()
  ...
  useEffect(() => {
    ...
    if (!supabase) return
    // Re-load settings whenever the authenticated user changes so that
    // a newly confirmed user gets their own settings (not a stale session's).
